...
#    endif /* Q_OS_SOLARIS */
                if (!access(d->ttyName.data(),
                            R_OK | W_OK)) { // checks availability based on permission bits
                    if (!geteuid()) {
...
gotpty:
    ...
    if (((st.st_uid != getuid())
        ...
        && !d->chownpty(true)) {
        qWarning() << "chownpty failed for device " << ptyName << "::" << d->ttyName
...
#        ifdef HAVE_STRUCT_UTMP_UT_ID
    strncpy(l_struct.ut_id, str_ptr + strlen(str_ptr) - sizeof(l_struct.ut_id),
