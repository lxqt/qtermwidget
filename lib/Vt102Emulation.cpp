...
Vt102Emulation::Vt102Emulation()
    : Emulation(), prevCC(0), _titleUpdateTimer(new QTimer(this)), _reportFocusEvents(false)
{
        ...
        for (int i = 0; i <= argc; i++) {
            if (epp())
                ...
                processToken(TY_CSI_PG(cc), 0, 0); // spec. case for ESC]>0c or ESC]>c
                     ...
                     && argv[i + 1] == 2) {
                // ESC[ ... 48;2;<red>;<green>;<blue> ... m -or- ESC[ ... 38;2;<red>;<green>;<blue>
                // ... m
                i += 2;
...
void Vt102Emulation::processToken(int token, wchar_t p, int q)
{
    ...
    case TY_CSI_PR('r', 67): /* IGNORED: DECBKM                   */
        ...
        // 1000,1001,1002,1003 Konsole currently supports mode 1000 (basic mouse press and release)
        // and mode 1002 (dragging the mouse).
...
void Vt102Emulation::sendMouseEvent(int cb, int cx, int cy, int eventType)
{
    ...
    // Note that if multiple extensions are enabled, the 1006 is used, so it's okay to check for
    // only that.
    ...
    char command[32];
    command[0] = '\0';
    // Check the extensions in decreasing order of preference. Encoding the release event above
    // assumes that 1006 comes first.
...
void Vt102Emulation::sendKeyEvent(QKeyEvent *event, bool fromPaste)
{
    ...
    if (_keyTranslator) {
        ...
        bool wantsAnyModifier =
                entry.state() & entry.stateMask() & KeyboardTranslator::AnyModifierState;
        ...
        else if ((modifiers & KeyboardTranslator::CTRL_MOD) && event->key() >= 0x40
                 && event->key() < 0x5f) {
...
{
    KeyboardTranslator::Entry entry = _keyTranslator->findEntry(Qt::Key_Backspace, Qt::NoModifier,
...
void Vt102Emulation::reportDecodingError()
{
    if (tokenBufferPos == 0 || (tokenBufferPos == 1 && (tokenBuffer[0] & 0xff) >= 32))
        return;
    qCDebug(qtermwidgetLogger) << "Undecodable sequence:"
