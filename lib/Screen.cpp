...
Character Screen::defaultChar =
        Character(' ', CharacterColor(COLOR_SPACE_DEFAULT, DEFAULT_FORE_COLOR),
...
//=STBM
{
    ...
    if (!(0 <= top && top < bot
          && bot < lines)) { // Debug()<<" setRegion("<<top<<","<<bot<<") : bad range.";
...
void Screen::copyFromScreen(Character *dest, int startLine, int count) const
{
    ...
    for (int line = startLine; line < (startLine + count); line++) {
        ...
        for (int column = 0; column < columns; column++) {
            ...
            dest[destIndex] =
                    screenLines[srcIndex / columns].value(srcIndex % columns, defaultChar);
...
void Screen::getImage(Character *dest, int size, int startLine, int endLine) const
{
    ...
    if (linesInScreenBuffer > 0)
        copyFromScreen(dest + linesInHistoryBuffer * columns,
                       startLine + linesInHistoryBuffer - history->getLines(), linesInScreenBuffer);
