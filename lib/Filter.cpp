...
TerminalImageFilterChain::TerminalImageFilterChain() : _buffer(nullptr), _linePositions(nullptr) { }
...
Filter::Filter() : _linePositions(nullptr), _buffer(nullptr) { }
...
void Filter::getLineColumn(int position, int &startLine, int &startColumn)
{
    ...
    for (int i = 0; i < _linePositions->count(); i++) {
        ...
        if (_linePositions->value(i) <= position && position < nextLine) {
            startLine = i;
            startColumn = string_width(
                    buffer()->mid(_linePositions->value(i), position - _linePositions->value(i))
                            .toStdWString());
            return;
        }
    }
...
// Note:  Altering these regular expressions can have a major effect on the performance of the
// filters used for finding URLs in the text, especially if they are very general and could match
// very long pieces of text. Please be careful when altering them.

// regexp matches:
// full url:
// protocolname:// or www. followed by anything other than whitespaces, <, >, ' or ", and ends
// before whitespaces, <, >, ', ", ], !, comma and dot
const QRegExp UrlFilter::FullUrlRegExp(
        QLatin1String("(www\\.(?!\\.)|[a-z][a-z0-9+.-]*://)[^\\s<>'\"]+[^!,\\.\\s<>'\"\\]]"));
// email address:
// [word chars, dots or dashes]@[word chars, dots or dashes].[word chars]
const QRegExp
        UrlFilter::EmailAddressRegExp(QLatin1String("\\b(\\w|\\.|-)+@(\\w|\\.|-)+\\.\\w+\\b"));

// matches full url or email address
const QRegExp UrlFilter::CompleteUrlRegExp(QLatin1Char('(') + FullUrlRegExp.pattern()
                                           + QLatin1Char('|') + EmailAddressRegExp.pattern()
                                           + QLatin1Char(')'));
