...
HistorySearch::HistorySearch(EmulationPtr emulation, const QRegExp &regExp, bool forwards,
...
void HistorySearch::search()
{
    ...
    if (!m_regExp.isEmpty()) {
        if (m_forwards) {
            found = search(m_startColumn, m_startLine, -1, m_emulation->lineCount())
...
{
    qDebug() << "search from" << startColumn << "," << startLine << "to" << endColumn << ","
             << endLine;
    ...
    while ((blockSize = qMin(10000, linesToRead - linesRead)) > 0) {
        ...
        int blockStartLine =
                m_forwards ? startLine + linesRead : endLine - linesRead - blockSize + 1;
        ...
        // The String that Emulator.writeToStream produces has a newline at the end, and so ends
        // with an empty line - we ignore that.
        ...
        if (matchStart > -1) {
            ...
            // Translate startPos and endPos to startColum, startLine, endColumn and endLine in
            // history.
            ...
            qDebug() << "m_foundStartColumn" << m_foundStartColumn << "m_foundStartLine"
                     << m_foundEndLine << "m_foundEndColumn" << m_foundEndColumn << "m_foundEndLine"
                     << m_foundEndLine;
