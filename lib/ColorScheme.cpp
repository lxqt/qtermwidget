...
ColorEntry ColorScheme::colorEntry(int index) const
{
    ...
    if (_randomTable != nullptr && !_randomTable[index].isNull()) {
        ...
        int hueDifference =
                range.hue ? QRandomGenerator::global()->bounded(range.hue) - range.hue / 2 : 0;
...
void ColorScheme::read(const QString &fileName)
{
    ...
    _description =
            s.value(QLatin1String("Description"), QObject::tr("Un-named Color Scheme")).toString();
...
void ColorScheme::readColorEntry(QSettings *s, int index)
{
    ...
    if (!ok) {
        qWarning().nospace() << "Invalid color value " << colorStr << " for " << colorName
                             << ". Fallback to black.";
        r = g = b = 0;
    }
...
ColorSchemeManager::ColorSchemeManager() : _haveLoadedAll(false) { }
...
bool ColorSchemeManager::loadColorScheme(const QString &filePath)
{
    ...
    if (scheme->name().isEmpty()) {
        // qDebug() << "Color scheme in" << filePath << "does not have a valid name and was not
        // loaded.";
        delete scheme;
        return false;
    }
