...
void TerminalDisplay::setScreenWindow(ScreenWindow *window)
{
    ...
    if (window) {
        ...
        //#warning "The order here is not specified - does it matter whether updateImage or
        // updateLineProperties comes first?"
...
unsigned short
        Konsole::vt100_graphics[32] = { // 0/8     1/9    2/10    3/11    4/12    5/13    6/14 7/15
            0x0020, 0x25C6, 0x2592, 0x2409, 0x240c, 0x240d, 0x240a, 0x00b0, 0x00b1, 0x2424, 0x240b,
            0x2518, 0x2510, 0x250c, 0x2514, 0x253c, 0xF800, 0xF801, 0x2500, 0xF803, 0xF804, 0x251c,
...
void TerminalDisplay::calDrawTextAdditionHeight(QPainter &painter)
{
    ...
    painter.drawText(test_rect, Qt::AlignBottom, LTR_OVERRIDE_CHAR + QLatin1String("Mq"),
                     &feedback_rect);
...
void TerminalDisplay::setVTFont(const QFont &f)
{
    ...
    if (!QFontInfo(font).fixedPitch()) {
        qDebug() << "Using a variable-width font in the terminal.  This may cause performance "
                    "degradation and display/alignment errors.";
    }
...
void TerminalDisplay::drawBackground(QPainter &painter, const QRect &rect,
                                     const QColor &backgroundColor, bool useOpacitySetting)
{
                                 ...
                                 bool &invertCharacterColor)
{
    ...
    if (!_cursorBlinking) {
        ...
        if (_cursorShape == Emulation::KeyboardCursorShape::BlockCursor) {
            ...
            painter.drawRect(
                    cursorRect.adjusted(penWidth / 2, penWidth / 2, -penWidth / 2, -penWidth / 2));
            if (hasFocus()) {
            ...
            painter.drawLine(cursorRect.left(), cursorRect.bottom(), cursorRect.right(),
                             cursorRect.bottom());
...
void TerminalDisplay::drawCharacters(QPainter &painter, const QRect &rect, const std::wstring &text,
                                     const Character *style, bool invertCharacterColor)
{
    ...
    if (font.bold() != useBold || font.underline() != useUnderline || font.italic() != useItalic
        || font.strikeOut() != useStrikeOut || font.overline() != useOverline) {
...
void TerminalDisplay::scrollImage(int lines, const QRect &screenWindowRegion)
{
    ...
    if (lines == 0 || _image == nullptr || !region.isValid()
        || (region.top() + abs(lines)) >= region.bottom() || this->_lines <= region.height())
        return;
...
void TerminalDisplay::processFilters()
{
    ...
    _filterChain->setImage(_screenWindow->getImage(), _screenWindow->windowLines(),
...
void TerminalDisplay::updateImage()
{
    ...
    if (!_image) {
        // Create _image.
        // The emitted changedContentSizeSignal also leads to getImage being recreated, so do this
        // first.
        updateImageSize();
    }
    ...
    for (y = 0; y < linesToUpdate; ++y) {
        ...
        if (!_resizing) // not while _resizing, we're expecting a paintEvent
            for (x = 0; x < columnsToUpdate; ++x) {
                ...
                if (dirtyMask[x]) {
                    ...
                    bool doubleWidth =
                            (x + 1 == columnsToUpdate) ? false : (newLine[x + 1].character == 0);
                    ...
                    for (len = 1; len < lln; ++len) {
                        ...
                        bool nextIsDoubleWidth = (x + len + 1 == columnsToUpdate)
                                ? false
        ...
        if (updateLine) {
            ...
            // repainted
            QRect dirtyRect = QRect(_leftMargin + tLx, _topMargin + tLy + _fontHeight * y,
    ...
    if (linesToUpdate < _usedLines) {
        dirtyRegion |=
                QRect(_leftMargin + tLx, _topMargin + tLy + _fontHeight * linesToUpdate,
                      _fontWidth * this->_columns, _fontHeight * (_usedLines - linesToUpdate));
    }
    _usedLines = linesToUpdate;

    if (columnsToUpdate < _usedColumns) {
        dirtyRegion |=
                QRect(_leftMargin + tLx + columnsToUpdate * _fontWidth, _topMargin + tLy,
                      _fontWidth * (_usedColumns - columnsToUpdate), _fontHeight * this->_lines);
    }
    ...
    if (_terminalSizeHint && isVisible()) {
        ...
        if (!_resizeWidget) {
            ...
            _resizeWidget->setStyleSheet(
                    QLatin1String("background-color:palette(window);border-style:solid;border-"
                                  "width:1px;border-color:palette(dark)"));
...
void TerminalDisplay::paintEvent(QPaintEvent *pe)
{
    ...
    if (!_backgroundImage.isNull()) {
        ...
        if (_backgroundMode
            == Stretch) { // scale the image without keeping its proportions to fill the screen
            ...
            paint.drawPixmap(r, _backgroundImage, _backgroundImage.rect());
                 ...
                 == Fit) { // if the image is bigger than the terminal, zoom it out to fit it
            QRect r = _backgroundImage.rect();
            qreal wRatio = static_cast<qreal>(cr.width()) / r.width();
            qreal hRatio = static_cast<qreal>(cr.height()) / r.height();
            if (r.width() > cr.width()) {
...
QRect TerminalDisplay::preeditRect() const
{
    ...
    return QRect(_leftMargin + _fontWidth * cursorPosition().x(),
                 _topMargin + _fontHeight * cursorPosition().y(), _fontWidth * preeditLength,
                 _fontHeight);
}
    ...
    while (iter.hasNext()) {
        ...
        for (int line = spot->startLine(); line <= spot->endLine(); line++) {
            ...
            if (spot->type() == Filter::HotSpot::Link) {
                ...
                // find the baseline (which is the invisible line that the characters in the font
                // sit on, with some having tails dangling below)
...
QRect TerminalDisplay::calculateTextArea(int topLeftX, int topLeftY, int startColumn, int line,
                                         int length)
{
    ...
    for (int y = luy; y <= rly; y++) {
        ...
        for (; x <= rlx; x++) {
                   ...
                   && isLineChar(c = _image[loc(x + len, y)].character) == lineDraw) // Assignment!
            {
                if (c)
                    unistr[p++] = c; // fontMap(c);
                if (doubleWidth) // assert((_image[loc(x+len,y)+1].character == 0)), see above if
                                 // condition
...
void TerminalDisplay::setScroll(int cursor, int slines)
{
    ...
    if (_scrollBar->minimum() == 0 && _scrollBar->maximum() == (slines - _lines)
    ...
    if (ev->button() == Qt::LeftButton) {
        ...
        if ((!_ctrlDrag || ev->modifiers() & Qt::ControlModifier) && selected) {
            ...
            _columnSelectionMode =
                    (ev->modifiers() & Qt::AltModifier) && (ev->modifiers() & Qt::ControlModifier);
...
void TerminalDisplay::mouseMoveEvent(QMouseEvent *ev)
{
    ...
    if (spot && spot->type() == Filter::HotSpot::Link) {
        ...
        if (spot->startLine() == spot->endLine()) {
            ...
            _mouseOverHotspotArea |= r;
            for (int line = spot->startLine() + 1; line < spot->endLine(); line++) {
                r.setCoords(0 * _fontWidth + leftMargin, line * _fontHeight + _topBaseMargin,
                            _columns * _fontWidth + leftMargin,
                            (line + 1) * _fontHeight + _topBaseMargin);
                _mouseOverHotspotArea |= r;
            }
            r.setCoords(0 * _fontWidth + leftMargin, spot->endLine() * _fontHeight + _topBaseMargin,
                        spot->endColumn() * _fontWidth + leftMargin,
...
void TerminalDisplay::extendSelection(const QPoint &position)
{
    ...
    QRect textBounds(tLx + _leftMargin, tLy + _topMargin, _usedColumns * _fontWidth - 1,
    ...
    if (!_wordSelectionMode && !_lineSelectionMode) {
        ...
        if (right.x() > 0 && !_columnSelectionMode) {
            ...
            if (i >= 0 && i <= _imageSize) {
                ...
                /* if (selClass == ' ')
                 {
                   while ( right.x() < _usedColumns-1 && charClass(_image[i+1].character) ==
                 selClass && (right.y()<_usedLines-1) &&
...
void TerminalDisplay::mouseReleaseEvent(QMouseEvent *ev)
{
    ...
    if (!_mouseMarks
        ...
        emit mouseSignal(ev->button() == Qt::MidButton ? 1 : 2, charColumn + 1,
                         charLine + 1 + _scrollBar->value() - _scrollBar->maximum(), 2);
    }
}
...
void TerminalDisplay::mouseDoubleClickEvent(QMouseEvent *ev)
{
    ...
    if (!_mouseMarks && !(ev->modifiers() & Qt::ShiftModifier)) {
        ...
        emit mouseSignal(0, pos.x() + 1, pos.y() + 1 + _scrollBar->value() - _scrollBar->maximum(),
                         0); // left button
        return;
    }
    ...
    QChar selClass = charClass(_image[i].character);
    {
        ...
        while (((x < _usedColumns - 1)
                || (endSel.y() < _usedLines - 1 && (_lineProperties[endSel.y()] & LINE_WRAPPED)))
               && charClass(_image[i + 1].character) == selClass) {
            i++;
            if (x < _usedColumns - 1)
                x++;
...
void TerminalDisplay::outputSuspended(bool suspended)
{
    ...
    if (!_outputSuspendedLabel) {
        ...
        _outputSuspendedLabel =
                new QLabel(tr("<qt>Output has been "
...
AutoScrollHandler::AutoScrollHandler(QWidget *parent) : QObject(parent), _timerId(0)
{
...
void AutoScrollHandler::timerEvent(QTimerEvent *event)
{
    ...
    QMouseEvent mouseEvent(QEvent::MouseMove, widget()->mapFromGlobal(QCursor::pos()), Qt::NoButton,
