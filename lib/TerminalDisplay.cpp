...
void TerminalDisplay::scrollImage(int lines, const QRect& screenWindowRegion)
{
    ...
    if (lines > 0)
    {
        ...
        Q_ASSERT((char*) lastCharPos + bytesToMove < (char*) (_image + (this->_lines * this->_columns)));
    ...
    {
        // check that the memory areas that we are going to move are valid
        Q_ASSERT((char*) firstCharPos + bytesToMove < (char*) (_image + (this->_lines * this->_columns)));
            ...
            for (x = 0; x < columnsToUpdate; ++x)
            {
                ...
                if (dirtyMask[x])
                {
                    ...
                    for (len = 1; len < lln; ++len)
                    {
                        ...
                        if (ch.foregroundColor != cf || ch.backgroundColor != _clipboard || ch.rendition != cr || !dirtyMask[x + len] || isLineChar(c) != lineDraw || nextIsDoubleWidth != doubleWidth)
                            break;
...
void TerminalDisplay::mouseMoveEvent(QMouseEvent* ev)
{
    ...
    if (dragInfo.state == diPending)
    {
        ...
        if (ev->x() > dragInfo.start.x() + distance || ev->x() < dragInfo.start.x() - distance || ev->y() > dragInfo.start.y() + distance || ev->y() < dragInfo.start.y() - distance)
        {
...
void TerminalDisplay::outputSuspended(bool suspended)
{
    ...
    if (!_outputSuspendedLabel)
    {
        ...
        _outputSuspendedLabel->setTextInteractionFlags(Qt::LinksAccessibleByMouse | Qt::LinksAccessibleByKeyboard);
