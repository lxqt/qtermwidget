...
void TerminalDisplay::fontChange(const QFont&)
{
    ...
    _needUpdateDrawPadding = true;
    update();
}

void TerminalDisplay::updateDrawPadding(QPainter& painter)
{
    ...
    _drawPaddingHeight = (feedback_rect.height() - _fontHeight) / 2;
    if(_drawPaddingHeight < 0) {
        _drawPaddingHeight = 0;
    }
    
    // update the original content
    _needUpdateDrawPadding = false;
    update();
}
                                     ...
                                     bool invertCharacterColor)
{
    ...
    else
    {
        ...
        if (_bidiEnabled)
            ...
            drawRect.setHeight(rect.height() + _drawPaddingHeight);
#if QT_VERSION >= 0x040800
            painter.drawText(drawRect, Qt::AlignBottom, LTR_OVERRIDE_CHAR + text);
#else
            //TODO: test the following modification with Qt 4.7 or below
            painter.drawText(drawRect, 0, LTR_OVERRIDE_CHAR + text);
#endif
        }
    }
}
...
void TerminalDisplay::paintEvent( QPaintEvent* pe )
{
    ...
    if(_needUpdateDrawPadding) {
        updateDrawPadding(paint);
    } else {
